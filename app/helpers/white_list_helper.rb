module WhiteListHelper
  ...
  def white_list_html_nokogiri(html, options = {})
    ...
    doc = Nokogiri::HTML(sanitize(doc.to_html, :tags => allowed_tags, :attributes => allowed_attributes), :fixup_tags => true)
    ...
    ss.each do |images|
      ...
      images[0].swap("<div class='t-entry-slides'><div class='t-entry-slides-container'>#{replacement}</div></div>")
