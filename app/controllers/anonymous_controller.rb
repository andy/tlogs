  ...
  def comment
    ...
    if @comment.errors.empty?
      ...
      if @entry.author.is_emailable? && @entry.author.email_comments? && (!current_user || @entry.user_id != current_user.id)
        Emailer.deliver_comment(@entry.author, @comment)
      end
      
      # отправляем комменатрий каждому пользователю
      users.each do |user|
        Emailer.deliver_comment_reply(user, @comment) if user.is_emailable? && user.email_comments? && user.id != @entry.author.id
      end
