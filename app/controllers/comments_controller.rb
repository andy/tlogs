  ...
  def create
    ...
    if @comment.errors.empty?
      ...
      if current_site.is_emailable? && current_site.email_comments? && (!current_user || current_site.id != current_user.id)
        Emailer.send_later(:deliver_comment, current_service, current_site, @comment)
      end
      
      # отправляем комменатрий каждому пользователю
      users.each do |user|
        Emailer.send_later(:deliver_comment_reply, current_service, user, @comment) if user.is_emailable? && user.email_comments? && user.id != current_site.id
      end
