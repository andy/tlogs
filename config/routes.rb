...
www_settings = lambda do |www|  
  ...
  www.namespace :console do |console|
    console.resources :users, :member => { :disable => :post, :destroy => :post, :invitations => :post, :mprevoke => :post, :mpgrant => :post, :restore => :post, :wipeout => :post, :suspect => :get, :reporter => :get, :changelogs => :get, :confirm => :post }
